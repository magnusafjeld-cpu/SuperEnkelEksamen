#!/usr/bin/env python3
"""
Setter sammen FIE432_Manual.html av kapittelfragmentene.
...
arbeidsform for å skrive flere kapitler i parallell.
    ...
    python3 tools/fie432-bygg-manual.py <fragmentmappe>
...
# Kapittelkartet. Må stemme med `parts` i js/subjects.js og med docs/fie432-kursplan.md.
DELER = [
    ("Del 0", "Grunnlaget", [0, 1, 2]),
    ("Del I", "Aktiva og avkastning", [3, 4]),
    ("Del II", "Aksjonær- og formuesbeskatning", [5, 6, 7, 8]),
    ("Del III", "Skatteteori", [9, 10, 11, 12, 13]),
    ("Del IV", "Personlig finans", [14, 15, 16, 17, 18]),
    ("Del V", "Eksamenshåndverk", [19]),
    ("Referanse", "Satser og formler", [20]),
]
ROT = pathlib.Path(__file__).resolve().parent.parent
MÅL = ROT / "FIE432_Manual.html"
...
<title>FIE432 Personlig økonomi — hele kurset</title>
<style>
  :root{ --ink:#1a1a1a; --paper:#fbf9f4; --plum:#9a4a1e; --rule:#d8d0c0; --box:#fbf1e8; }
  ...
  .callout.mech{ background:var(--box); border-color:#f0d9c4; }
...
<div class="cover">
  <h1>FIE432 Personlig økonomi</h1>
  <p class="tagline">Skattesystemet, aksjonær- og formuesbeskatningen, skatteteorien og husholdningens finansbeslutninger — hele pensum, skrevet for å stå alene.</p>
  <div class="meta">NHH, høsten 2026. Kursansvarlig Guttorm Schjelderup; finansdelen ved Ole-Andreas Næss.
  Eksamen er fire timers flervalg i Wiseflow med minuspoeng for feil svar. Manualen er sannhetskilden for
  studieplanen, quizen, flashcardene og drillene i appen.</div>
</div>
"""
...
def main():
    ...
    if not mappe or not mappe.is_dir():
        sys.exit("bruk: python3 tools/fie432-bygg-manual.py <fragmentmappe>")
